...
# Importing client from a module named "client"
from . import client
...
def fetch_and_clean_urls(domain, extensions, stream_output, proxy, placeholder, output_dir):
    """
    ...
    urls_with_params = [url for url in cleaned_urls if "?" in url]
    if not urls_with_params:
        logging.info(f"{Fore.YELLOW}[INFO]{Style.RESET_ALL} {Fore.RED}No URLs{Style.RESET_ALL} with parameters found for {Fore.CYAN + domain + Style.RESET_ALL} {Fore.RED}>_<{Style.RESET_ALL}")
        return
    else:
        logging.info(f"{Fore.YELLOW}[INFO]{Style.RESET_ALL} Found {Fore.GREEN + str(len(urls_with_params)) + Style.RESET_ALL} URLs with parameters")
...
def main():
    ...
    parser.add_argument("-x", "--proxy", help="Set the proxy address for web requests.",default=None)
    parser.add_argument("-p", "--placeholder", help="Placeholder for parameter values/", default="FUZZ")
    parser.add_argument("-q", "--quiet", action="store_true", help="Hides all output except for results.")
    parser.add_argument("-o", "--output", help="Path where resulting text files should be stored.", default="results")
...
/_/                          /_/                    
                    ...
                    with <3 by @0xasm0d3us & @TargetPackage
    """
